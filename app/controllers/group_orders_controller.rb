class GroupOrdersController < ApplicationController
  ...
  before_action :set_group_order, only: [:show, :destroy, :advance_status, :update_discount, :update_discount_from_total, :update_bank_account]
  ...
  def update_discount_from_total
    ...
    if grand_total > 0 && total_paid > 0 && total_paid <= grand_total
      ...
      redirect_to @group_order, alert: "El monto debe ser mayor a 0 y menor o igual al total ($#{grand_total.round(2)})."
    end
  end
